<template>
	<view class="preview" v-if="currentInfo">
		...
		<view class="mask" v-if="maskState">
			...
			<view class="footer">
				<view class="box">
					<uni-icons type="info" size="23" @click="clickInfo"></uni-icons>
			...
			<view class="infoPopup">
				...
				<scroll-view scroll-y="true">
					<view class="content">
						...
						<view class="row" v-if="currentInfo">
		...
		<uni-popup ref="scorePopup" :is-mask-click="false">
			<view class="scorePopup">
				...
				<view class="content">
					<uni-rate v-model="userScore" allow-half="true" :disabled="isScore" disabled-color="#FFCA3E" />
...
<script setup>
	...
	classList.value = storgClassList.map(item => {
		return {
			...item,
			picurl: item.smallPicurl.replace("_small.webp", ".jpg") //往数组中新增数据将原来的后缀改为jpg格式
		}
	})

	//通过分类页面的id数据传递之后再通过findIndex函数来获取当前图片在数组中的索引值
	onLoad(async (e) => {
		currentId.value = e.id;
		//解决preview页面分享后用户无缓存情况，仅仅显示一张图片
		if(e.type == 'share'){
