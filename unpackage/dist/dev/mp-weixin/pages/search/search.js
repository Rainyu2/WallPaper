...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const onSearch = () => {
      ...
      searchData();
      common_vendor.index.__f__("log", "at pages/search/search.vue:95", queryParams.value.keyword);
    };
    const onClear = () => {
      initParams();
    };
    ...
    const searchData = async () => {
      try {
        ...
        if (res.data.length == 0 && classList.value.length == 0)
          noSearch.value = true;
        common_vendor.index.__f__("log", "at pages/search/search.vue:132", res);
      } finally {
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        m: common_vendor.p({
          mode: "search",
          icon: "/static/images/noSearch.png"
        })
      } : common_vendor.e({
